...
public class Utilities {
  ...
  public static boolean canInteract(AbstractHorseEntity horse, Entity entity) {
    if (horse.getOwnerReference() == null) {
      ...
      return horse.getOwnerReference().getUuid().equals(entity.getUuid())
          ...
          || Permissions.check(
              entity, PrivateHorses.MOD_ID + ".interact", 4); // Взаємодія дозволена
    }
  }
  ...
  public static void sendOwnershipNotification(AbstractHorseEntity horse, Entity entity) {
    ...
    if (entity instanceof ServerPlayerEntity player
        ...
        && server != null
        && server.getApiServices().nameToIdCache() instanceof NameToIdCache cache) {
      ...
      horseOwnerName =
          horseOwner
              .map(playerConfigEntry -> "§e" + playerConfigEntry.name())
              .orElse("§7§kunknown");
      ...
      if (polymer_loaded && !ModConfig.getBooleanValue("ignore_polymer"))
        ...
        message =
            Text.literal(getStringValue("message.owned_by").formatted(horseName, horseOwnerName));

      player.sendMessage(message, true);
    }
  }

  public static void succeedTransferMessage(
      PlayerEntity newOwner, PlayerEntity oldOwner, AnimalEntity animal) {
    ...
    if (polymer_loaded && !ModConfig.getBooleanValue("ignore_polymer")) {
      oldOwnerMessage =
          Text.translatable("message.private-horses.new_owner", newOwnerName, animalName);
      newOwnerMessage =
          Text.translatable("message.private-horses.transfer", oldOwnerName, animalName);
    } else {
      oldOwnerMessage =
          Text.literal(getStringValue("message.new_owner").formatted(newOwnerName, animalName));
      newOwnerMessage =
          Text.literal(getStringValue("message.transfer").formatted(oldOwnerName, animalName));
    }
  ...
  public static void showParticles(
      ServerWorld world, Entity entity, ParticleEffect particleEffect) {
